...
- 单文件 Cloudflare Worker：`worker.js`，前后端全部逻辑都在这个文件里。
  ...
  - 页面由 `renderHTML()` 返回的一个巨型模板字符串组成，内联全部 HTML/CSS/JS。模板区起点锚点为 `function renderHTML`，终点为 ``</html>`;`` 闭合行；**一律按锚点文本定位，不要用行号**（行号会随改动漂移）。
- 无 package.json、无构建步骤、无测试套件。唯一的自动校验 = 复制成临时 `.mjs` 后 `node --check`。
...
1. **失败路径/加固类改动 → 以静态门 + 读码为准**：`node --check`、针对性 grep 断言（数量/位置/可达性）、改动前后语义逐处读码核对。交付说明中标注"无手工验证通道，以静态门 + 读码为准"，不派人工测试项。
...
3. **禁止包管理器与运行时**：npm/pnpm/yarn/npx/wrangler 一律不执行。
